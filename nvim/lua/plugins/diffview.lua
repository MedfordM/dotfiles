return {
  ...
  keys = {
    ...
    { '<leader>gdc',
      function()
        if vim.regex.match_str('\b[0-9a-f]{5,40}\b') == nil then
          require('diffview').file_history(nil, vim.fn.expand('<cword>'))
        else
          require('diffview').open(vim.fn.expand('<cword>'))
        end
      end,
    desc = 'Diff object under cursor' },
  },
  opts = {
    ...
    keymaps = {
      ...
      view = {
        ...
        { "n", "q",           function() require('diffview').close()                               end, { desc = 'Close diff' } },
        ...
        { "n", "g?",          function() require('diffview.actions').help({ "view", "view" })      end, { desc = "Open the help panel" } },
      },
      diff1 = {
      ...
      diff2 = {
        ...
        { "n", "X",  function() require('diffview.actions').restore_entry()           end, { desc = "Restore entry to the state on the left side" } },
        { {"n", 'v'}, "s",  function() vim.cmd('Git stage_hunk')      end, { desc = "Stage / unstage the selected entry" } },
        { "n", "q",  function() require('diffview').close()                           end, { desc = 'Close diff' } },
