return {
  {
    ...
    config = function(_, opts)
      ...
      StatusColumn.build = function()
        return table.concat(vim.tbl_flatten({ "%@v:lua.ScLa@", number(), "%T", fold(), "%@v:lua.StatusColumn.signClickHandler@%s%T", border(), padding() }))
      end

      StatusColumn.signClickHandler = function(minWinId, numClicks, mouseButton, modifiers)
        local line = vim.fn.getmousepos().line
        print(line)
        line = line - 1
        if mouseButton == "l" then
          vim.diagnostic.open_float({scope = 'line', pos = { line, 0 }})       -- Open diagnostic float on left click
        elseif mouseButton == "r" then
